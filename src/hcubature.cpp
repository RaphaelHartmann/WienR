...
void increment(std::vector<bool>& index, int k, double lambda, int n, int* c, double* temp) {
    ...
    if (first_zero == static_cast<int>(index.size())) {
        index.flip();
        for (long unsigned int j = 0; j != index.size(); j++) temp[c[j] - 1] *= -1;
...
void signcombos(int k, double lambda, int n, std::vector<double*>& p) {
    int* c = (int*)malloc(k * sizeof(int));
    for (int i = 1; i != choose(n, k) + 1; i++) {
        double* temp = (double*)calloc(n, sizeof(double));
        combination(c, n, k, i);
...
void clean_GenzMalik(GenzMalik& g) {
    for (int j = 0; j != 4; j++)
        for (int i = 0; i != static_cast<int>(g.p[j].size()); i++) if (g.p[j][i]) free(g.p[j][i]);
}
...
void integrate_GenzMalik(GenzMalik g, int n, const double* a, const double* b, one_d& out, void* pars, int integrand(unsigned dim, const double* x, void* p, unsigned fdim, double* retval)) {
    ...
    double f4 = 0.0;
    for (int i = 0; i != static_cast<int>(g.p[2].size()); i++) {
    ...
    for (int i = 0; i != static_cast<int>(g.p[3].size()); i++) {
...
int hcubature(int integrand(unsigned dim, const double* x, void* p, unsigned fdim, double* retval), void* pars, unsigned n, const double* a, const double* b,
    ...
    long unsigned int numevals = (n == 1) ? 15 : 1 + 4 * n + 2 * n * (n - 1) + pow(2, n);
    long unsigned int evals_per_box = numevals;
