...
gradWienerPDF <- function(t,
  ...
  if (length(indW) > 0) {
    ...
    out$da[indW] <- temp$da; out$dv[indW] <- temp$dv; out$dw[indW] <- temp$dw; out$dt0[indW] <- -temp2$deriv; out$dsv[indW_sv0] <- temp$dsv[ind_sv0]; out$dsw[indW] <- NaN; out$dst[indW] <- NaN;
  }
  if (length(indD) > 0){
