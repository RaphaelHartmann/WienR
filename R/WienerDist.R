...
#'
#' Calculate the first-passage time cumulative distribution function of the diffusion model.
...
WienerCDF <- function(t,
  ...
  if (length(indW) > 0) {
    ...
    out$cdf[indW] <- temp$cdf
    out$logcdf[indW] <- temp$logcdf
  } 
  if (length(indD) > 0){
