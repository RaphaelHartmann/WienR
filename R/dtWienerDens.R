...
#' Partial derivative of the Wiener diffusion probability density function with respect to first-passage time
#'
#' Calculate the partial derivative of the Wiener diffusion probability density function with respect to the first-passage time t.
...
#'     \item \code{derivln}: the derivatives of the log-transformed PDF with respect to a,
#'     \item \code{call}: the function call.
#'   }
#' @examples
#' dtWienerPDF(t = 1.2, response = "upper", a = 1.1, v = 13, w = .6, precision = NULL, K = NULL)
#' @author Raphael Hartmann
#' @useDynLib "WienR", .registration=TRUE
#' @export
dtWienerPDF <- function(t,
                        response,
                        a,
                        v,
                        w,
  ...
  if(!is.null(K)) {
    ...
    if(K %% 1 != 0) stop("K must be an integer") else K <- as.integer(K)
  }
  ...
  derivative <- list(deriv = out$deriv, derivln = out$deriv_ln, time = out$time, call = match.call())
